...
class Loader {
	...
	public function create_action_links( $links ) {
		...
		if ( in_array( $license_status, array( 'not_active', 'invalid' ), true ) ) {
			$links[] = '<a href="' . esc_url( self::get_action_links( 'activate' ) ) . '">' . esc_html__( 'Activate', 'neve-pro-addon' ) . '</a>';
		} elseif ( $license_status === 'active_expired' ) {
			$links[] = '<a href="' . esc_url( self::get_action_links( 'renew' ) ) . '" target="_blank" rel="external noopener noreferrer">' . esc_html__( 'Renew', 'neve-pro-addon' ) . '</a>';
		} elseif ( $this->get_license_type() < 3 ) {
			$links[] = '<a href="' . esc_url( self::get_action_links( 'upgrade' ) ) . '" target="_blank" rel="external noopener noreferrer">' . esc_html__( 'Upgrade', 'neve-pro-addon' ) . '</a>';
		}
	...
	public function register_sidebars() {
		for ( $i = 1; $i <= 9; $i++ ) {
			$sidebar_settings = array(
				'name'          => esc_html__( 'Widget Area', 'neve-pro-addon' ) . ' ' . $i,
				'id'            => 'widget-area-' . $i,
	...
	/**
	 * Check if the user is a new user of the theme after a certain version.
	 *
	 * @param string $version Theme version in semver format.
	 *
	 * @return bool
	 */
	public static function is_new_user_after_theme_version( $version ) {
		$new_user_since = get_option( 'neve_user_since' );

		if ( ! $new_user_since || $new_user_since === 'unknown' ) {
			return false;
		}
	...
	/**
	 * Checks if the user is new after v4 of the theme.
	 *
	 * @return bool
	 */
	public static function is_new_user_after_theme_v4() {
		return self::is_new_user_after_theme_version( '3.8.16' );
	}
}
