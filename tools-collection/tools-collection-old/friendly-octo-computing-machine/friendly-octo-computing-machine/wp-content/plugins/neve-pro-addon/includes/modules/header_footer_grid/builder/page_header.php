...
class Page_Header extends Abstract_Builder {
	...
	public function init() {
		...
		$this->set_property( 'title', __( 'Page Header', 'neve-pro-addon' ) );
		$this->set_property(
			'description',
			sprintf(
				/* translators: %1$s: the module name ('Page Header') */
				esc_html__( 'Design your %1$s by dragging, dropping and resizing all the elements in real-time.', 'neve-pro-addon' ),
				$this->get_property( 'title' )
			)
			. ' '
			. sprintf(
					/* translators: %s: the link to documentation. */
				'<br/><a target="_blank" href="https://docs.themeisle.com/article/1057-header-booster-documentation">%s</a>',
				esc_html__( 'Read full documentation', 'neve-pro-addon' )
			)
		);
		$this->set_property(
			'instructions_array',
			array(
				'description' => sprintf(
					/* translators: %1$s: the module name ('Page Header'), %2$s: the add new component button sign (`+` is used). */
					esc_attr__( 'Welcome to the %1$s builder! Click the %2$s button to add a new component or follow the Quick Links.', 'neve-pro-addon' ),
					$this->get_property( 'title' ),
					'+'
				),
				'quickLinks'  => array(
					'hfg_page_header_layout_top_background' => array(
						'label' => esc_html__( 'Change Top Row Color', 'neve-pro-addon' ),
		...
		$this->devices           = array(
			'desktop' => __( 'Desktop', 'neve-pro-addon' ),
		);
		$this->devices['mobile'] = __( 'Mobile', 'neve-pro-addon' );
	 ...
	 *
	 * @deprecated 4.0.0 - We enabled the page header by default.
	 * 
	 * @return boolean
	 */
	...
	protected function get_rows() {
		return array(
			'top'    => array(
				'title'       => esc_html__( 'Page Header Top', 'neve-pro-addon' ),
				'description' => $this->get_property( 'description' ),
			),
			'bottom' => array(
				'title'       => esc_html__( 'Page Header Bottom', 'neve-pro-addon' ),
				'description' => $this->get_property( 'description' ),
			),
		);
	}
	...
	private function add_archives_controls( WP_Customize_Manager $wp_customize, $post_types ) {
		...
		$wp_customize->add_setting(
			...
			[
				...
				'default'           => esc_html__( 'Show on Archives', 'neve-pro-addon' ),
			]
		);
		...
		$heading = new Heading(
			...
			[
				...
				'label'            => esc_html__( 'Show on Archives', 'neve-pro-addon' ),
				'description'      => esc_html__( 'Select taxonomies', 'neve-pro-addon' ),
				'accordion'        => true,
				'expanded'         => true,
	...
	private function add_singles_controls( WP_Customize_Manager $wp_customize, $post_types ) {
		...
		$wp_customize->add_setting(
			...
			[
				'transport'         => 'refresh',
				'sanitize_callback' => 'sanitize_text_field',
				'default'           => esc_html__( 'Show on Singles', 'neve-pro-addon' ),
			]
		);
		...
		$heading = new Heading(
			...
			[
				'type'             => 'neve_customizer_heading',
				'section'          => self::GLOBAL_SETTINGS_SECTION,
				'label'            => esc_html__( 'Show on Singles', 'neve-pro-addon' ),
				'description'      => esc_html__( 'Select post types', 'neve-pro-addon' ),
				'accordion'        => true,
				'expanded'         => true,
				'controls_to_wrap' => $controls_wrap,
	...
	public function customize_register( WP_Customize_Manager $wp_customize ) {
		...
		$wp_customize->add_section(
			...
			[
				'priority' => 100,
				'title'    => esc_html__( 'Global Page Header Settings', 'neve-pro-addon' ),
				'panel'    => $this->panel,
			]
		);
		...
		if ( Loader::has_compatibility( 'link_control' ) ) {
			...
			$link_control = new \Neve\Customizer\Controls\React\Link_Control(
				...
				[
					'type'        => 'neve_link',
					'section'     => self::GLOBAL_SETTINGS_SECTION,
					'label'       => esc_html__( 'Create a custom page header', 'neve-pro-addon' ),
					'url'         => $layouts_url,
					'description' => esc_html__( 'Customize further the display of your page headers using custom layouts', 'neve-pro-addon' ),
				]
			);
		...
		if ( Loader::has_compatibility( 'page_header_support' ) ) {
			...
			$wp_customize->add_control(
				...
				[
					'type'    => 'text',
					'section' => self::GLOBAL_SETTINGS_SECTION,
					'label'   => esc_html__( 'Available Components', 'neve-pro-addon' ),
				]
			);
		}
