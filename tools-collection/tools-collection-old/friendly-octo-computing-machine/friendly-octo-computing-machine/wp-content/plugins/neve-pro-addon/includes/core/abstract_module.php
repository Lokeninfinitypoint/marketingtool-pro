...
abstract class Abstract_Module implements Module_Interface {
	...
	/**
	 * Enable and hide from Dashboard UI flag.
	 * 
	 * @var bool
	 * 
	 * @since 4.0.0 - added this flag for some modules which should be enabled by default and not shown in the dashboard.
	 */
	protected $force_enabled = false;
	...
	public function __construct() {
		...
		if ( $this->force_enabled ) {
			add_filter( 'option_' . $this->status_key, '__return_true' );
		}

		add_action( 'init', [ $this, 'setup_labels' ], 0 );
	 ...
	 * 
	 * @changed 4.0.0 - enabled by default & hidden from dashboard when force_enabled is true.
	 * 
	 * @return bool
	 */
	 ...
	 * @version 4.1.0
	 */
	abstract public function setup_labels();
	...
	public function get_module_info() {
		$info = array(
			$this->slug => array(
				...
				'hide'              => $this->force_enabled,
			),
		);

		// Hide the module if it has dependent plugins and they are not active.
		if ( is_array( $this->dependent_plugins ) && ! empty( $this->dependent_plugins ) ) {
			if ( $this->check_dependent_plugins() !== false ) {
				$info[ $this->slug ]['hide'] = true;
			}
		}
	...
	private function check_theme_version() {
		if ( ! $this->is_min_req_theme_version() ) {
			...
			return sprintf(
				...
				esc_url( $link ),
				esc_html__( 'You need to update the theme in order to use this module!', 'neve-pro-addon' )
			);
		}
	...
	protected function check_dependent_plugins() {
		...
		foreach ( $this->dependent_plugins as $slug => $plugin ) {
			...
			if ( ! is_plugin_active( $plugin['path'] ) ) {
				...
				$stat_strings = array(
					'install'  => __( 'installing', 'neve-pro-addon' ),
					'activate' => __( 'activating', 'neve-pro-addon' ),
				);
				...
				if ( $need_admin_action ) {
					...
					/* translators: %s - Required action text */
						...
						/* translators: %s - plugin to activate */
							__( 'The module requires %s plugin. Ask your admin to activate it.', 'neve-pro-addon' ),
				...
				return sprintf(
					...
					sprintf(
						/* translators: %s - plugin to activate */
						__( 'The module requires %s plugin.', 'neve-pro-addon' ),
						$stat_strings[ $state ] . ' ' . $plugin['name']
					)
				);
			}
