...
class Single_Product extends Base_View {
		...
		add_action(
			...
			function () {
				the_widget(
					...
					array(
						'number' => 6,
						'title'  => __( 'Recently Viewed', 'neve-pro-addon' ),
					),
					array(
	...
	public function add_gallery_classes() {
		...
		$product_video  = get_post_meta( $product->get_id(), 'neve_meta_product_video_link', true );
		if ( empty( $attachment_ids ) && empty( $product_video ) ) {
			return false;
		}
	...
	public function render_product_navigation() {
		...
		// Next text
		$next_text = esc_html__( 'Previous Product', 'neve-pro-addon' );
		$next_text = apply_filters( 'neve_woo_nav_next_text', $next_text );

		// Prev text
		$prev_text = esc_html__( 'Next Product', 'neve-pro-addon' );
