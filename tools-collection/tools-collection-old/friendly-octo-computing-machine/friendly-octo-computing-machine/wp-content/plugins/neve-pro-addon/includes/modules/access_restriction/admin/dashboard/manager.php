...
class Manager {
	...
	private function get_ui_form_controls() {
			...
			switch ( $args['group'] ) {
				case Post::GROUP:
					...
					if ( $post_type_obj instanceof \WP_Post_Type ) {
						...
						$label = sprintf( __( 'Deleted Post Type (%s)', 'neve-pro-addon' ), $post_type );
					}
					break;
				...
				case Term::GROUP:
					...
					if ( $taxonomy_obj instanceof \WP_Taxonomy ) {
						...
						$label = sprintf( __( 'Deleted Taxonomy (%s)', 'neve-pro-addon' ), $taxonomy );
					}
					break;
				default:
		...
		$restriction_behavior_fields = [
			Storage_Adapter::SETTING_KEY_RESTRICTION_BEHAVIOR => [
				'label'         => esc_html__( 'For logged out users:', 'neve-pro-addon' ),
				'default_value' => '',
				'options'       => [
					[
						'label' => __( 'Show 404 page', 'neve-pro-addon' ),
						'value' => Storage_Adapter::RESTRICT_BEHAVIOR_404_PAGE,
					],
					[
						'label' => __( 'Show default Wordpress login page', 'neve-pro-addon' ),
						'value' => Storage_Adapter::RESTRICT_BEHAVIOR_DEFAULT_WP_LOGIN,
					],
					[
						'label' => __( 'Show custom login page', 'neve-pro-addon' ),
			...
			Storage_Adapter::SETTING_KEY_CUSTOM_LOGIN_PAGE_ID => [
				'label'   => esc_html__( 'Custom login page:', 'neve-pro-addon' ),
		...
		return [
			'content_types'        => [
				'title'  => __( 'Content Types', 'neve-pro-addon' ),
				'icon'   => '',
				'fields' => $content_type_fields,
			],
			'restriction_behavior' => [
				'title'  => __( 'Restriction Behavior', 'neve-pro-addon' ),
				'icon'   => '',
				'fields' => $restriction_behavior_fields,
			],
			'sidebar'              => [
				'title'  => __( 'General', 'neve-pro-addon' ),
				'icon'   => '',
				'fields' => [],
			],
		];
	...
	public function enqueue() {
		...
		wp_set_script_translations( 'neve-access-restriction', 'neve-pro-addon' );
	}
